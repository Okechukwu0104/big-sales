...
export const TrustBadges = () => {
  const badges = [
    ...
    },
    {
      icon: Award,
      title: "Best Prices",
      description: "We find you the most affordable deals"
    },
    {
      icon: RefreshCw,
      title: "Easy Returns",
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4">
          {badges.map((badge, index) => (
            <div key={index} className="flex flex-col items-center text-center p-4 sm:p-6 rounded-2xl bg-card border border-border/50 hover:shadow-lg hover:border-primary/20 transition-all duration-300 group">
              <div className="w-12 h-12 sm:w-16 sm:h-16 rounded-full bg-primary/5 flex items-center justify-center mb-3 sm:mb-4 group-hover:scale-110 group-hover:bg-primary/10 transition-all duration-300">
                <badge.icon className="h-5 w-5 sm:h-7 sm:w-7 text-primary" />
              </div>
              <h3 className="font-bold text-sm sm:text-base text-foreground mb-1 sm:mb-2">{badge.title}</h3>
              <p className="text-[10px] sm:text-xs text-muted-foreground leading-relaxed">{badge.description}</p>
            </div>
          ))}
        </div>
      </div>
